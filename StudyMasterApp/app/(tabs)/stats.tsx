...
export default function StatsScreen() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      weekData.push({ day: DAYS[i], hours: dayHours, date: dayDate, isToday });
    }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.chart}>
        {weeklyData.map((day, i) => (
          <View key={i} style={[styles.barColumn, day.isToday && styles.barColumnToday]}>
            <View style={styles.barContainer}>
              <View 
                style={[
                  styles.bar, 
                  { 
                    height: `${Math.min((day.hours / maxHours) * 100, 100)}%`,
                    backgroundColor: day.isToday ? '#10b981' : day.hours > 0 ? '#6366F1' : '#E2E8F0'
                  }
                ]} 
              />
            </View>
            <Text style={[styles.dayLabel, day.isToday && styles.dayLabelToday]}>{day.day}</Text>
            <Text style={[styles.dateLabel, day.isToday && styles.dateLabelToday]}>{day.date}</Text>
            <Text style={[styles.hoursLabel, day.isToday && styles.hoursLabelToday]}>
...
const styles = StyleSheet.create({
  ...
  barColumn: { alignItems: 'center', flex: 1, paddingVertical: 8 },
  barColumnToday: { backgroundColor: '#ECFDF5', borderRadius: 12, marginHorizontal: -4, paddingHorizontal: 4 },
  barContainer: { height: 100, width: 28, backgroundColor: '#E2E8F0', borderRadius: 14, justifyContent: 'flex-end' },
  bar: { width: '100%', borderRadius: 14 },
  dayLabel: { fontSize: 12, fontWeight: '600', color: '#64748B', marginTop: 8 },
  dayLabelToday: { color: '#059669', fontWeight: 'bold' },
  dateLabel: { fontSize: 10, color: '#94A3B8', marginTop: 2 },
  dateLabelToday: { color: '#059669' },
  hoursLabel: { fontSize: 11, color: '#64748B', marginTop: 4, fontWeight: '600' },
  hoursLabelToday: { color: '#059669', fontWeight: 'bold', fontSize: 12 },
